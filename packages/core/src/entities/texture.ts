...
export function textureInit(
  ...
  const [textureFormat, textureCompressed] = mapTextureFormat(gl, format);

  if (textureCompressed && !mipmaps.length) {
  ...
  for (let i = 0; i < mipmaps.length + 1; i++) {
    const mipmapData = i ? mipmaps[i - 1] : data;
